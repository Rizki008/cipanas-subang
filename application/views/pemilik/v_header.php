<body>
	<div class="container-xxl position-relative bg-white d-flex p-0">

		<!-- Sidebar Start -->
		<div class="sidebar pe-4 pb-3">
			<nav class="navbar bg-light navbar-light">
				<a href="<?= base_url('pemilik') ?>" class="navbar-brand mx-4 mb-3">
					<h3 class="text-primary"><i class="fa fa-solid fa-bath"></i>CIPANAS</h3>
				</a>
				<div class="d-flex align-items-center ms-4 mb-4">
					<div class="position-relative">
						<img class="rounded-circle" src="<?= base_url() ?>backend/img/user.jpg" alt="" style="width: 40px; height: 40px;">
						<div class="bg-success rounded-circle border border-2 border-white position-absolute end-0 bottom-0 p-1"></div>
					</div>
					<div class="ms-3">
						<h6 class="mb-0"><?= $this->session->userdata('nama_admin'); ?></h6>
						<span>Pemilik</span>
					</div>
				</div>
				<div class="navbar-nav w-100">
					<a href="<?= base_url('pemilik') ?>" class="nav-item nav-link <?php if ($this->uri->segment(1) == 'pemilik' and $this->uri->segment(2) == "") {
																						echo "active";
																					} ?>"><i class="fa fa-tachometer-alt me-2"></i>Dashboard</a>
					<a href="<?= base_url('laporan') ?>" class="nav-item nav-link <?php if ($this->uri->segment(1) == 'laporan') {
																						echo "active";
																					} ?>"><i class="fa fa-chart-bar me-2"></i>Laporan</a>
				</div>
			</nav>
		</div>
		<!-- Sidebar End -->